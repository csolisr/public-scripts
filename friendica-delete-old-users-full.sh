...
tmpfile=/tmp/friendica-delete-old-users-full.csv
tmplock=/tmp/friendica-delete-old-users-full.tmp
...
loop() {
	...
	mapfile -t resultarray < <("${dbengine}" "${db}" -N -B -q -e "\
		create temporary table tmp_post_thread (select \`uri-id\` from \`post-thread\` where (\`owner-id\` = ${id} or \`author-id\` = ${id} or \`causer-id\` = ${id}) and (\`owner-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`author-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`causer-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`))) and (\`created\` < CURDATE() - INTERVAL ${period} and \`received\` < CURDATE() - INTERVAL ${period})); delete ignore h.* from \`post-thread\` h inner join \`tmp_post_thread\` t where h.\`uri-id\` = t.\`uri-id\`; select row_count(); \
		create temporary table tmp_post_thread_user (select \`uri-id\` from \`post-thread-user\` where (\`owner-id\` = ${id} or \`author-id\` = ${id} or \`causer-id\` = ${id}) and (\`owner-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`author-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`causer-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`))) and (\`created\` < CURDATE() - INTERVAL ${period} and \`received\` < CURDATE() - INTERVAL ${period})); delete ignore r.* from \`post-thread-user\` r inner join \`tmp_post_thread_user\` t where r.\`uri-id\` = t.\`uri-id\`; select row_count(); \
		create temporary table tmp_post_user (select \`id\` from \`post-user\` where (\`owner-id\` = ${id} or \`author-id\` = ${id} or \`causer-id\` = ${id}) and (\`owner-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`author-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`causer-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`))) and (\`created\` < CURDATE() - INTERVAL ${period} and \`received\` < CURDATE() - INTERVAL ${period})); delete ignore u.* from \`post-user\` u inner join \`tmp_post_user\` t where u.\`id\` = t.\`id\`; select row_count(); \
		delete ignore from \`post-tag\` where cid = ${id}; select row_count(); \
		create temporary table tmp_post (select \`uri-id\` from \`post\` where (\`owner-id\` = ${id} or \`author-id\` = ${id} or \`causer-id\` = ${id}) and (\`owner-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`author-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`)) and \`causer-id\` not in (select \`id\` from \`contact\` where \`uri-id\` in (select \`uri-id\` from \`contact\` where \`self\`))) and (\`created\` < CURDATE() - INTERVAL ${period} and \`received\` < CURDATE() - INTERVAL ${period})); delete ignore p.* from \`post-content\` p inner join \`tmp_post\` t where p.\`uri-id\` = t.\`uri-id\`; select row_count(); \
		delete ignore p.* from \`post-counts\` p inner join \`tmp_post\` t where p.\`uri-id\` = t.\`uri-id\`; select row_count(); \
