...
while read -r i; do
	...
	if [[ $(uname -n) == "${serveruname}" ]]; then
		while read -r j; do
			...
			diff <(sed -e "s/friendica.example.net/${serverurl}/g" -e "s/_OVERRIDES_/${overrides}/g" -e "s/#&>/\&\>/g" -e "s/\(token=\${.*:-\"\)[0-9a-f]*\"/\1${credential}\"/g" "${i}" | shfmt -mn | shfmt -s) <(shfmt -s "${j}")
			...
			sed -e "s/friendica.example.net/${serverurl}/g" -e "s/_OVERRIDES_/${overrides}/g" -e "s/#&>/\&\>/g" "${i}" -e "s/\(token=\${.*:-\"\)[0-9a-f]*\"/\1${credential}\"/g" | sudo tee "${j}" &>/dev/null
			shfmt -mn -w "${j}"
	...
	while read -r k; do
		...
		echo "${k}"
		#Show the differences between the current target file and the modified file
		diff <(sed -e "s/friendica.example.net/${serverurl}/g" -e "s/_OVERRIDES_/${overrides}/g" -e "s/\(token=\${.*:-\"\)[0-9a-f]*\"/\1${credential}\"/g" "${i}") "${k}"
		#Write the modified file to the target file, with the following modifications:
		#- Replace the placeholder server URL
		#- Replace the corresponding file credentials for the ones in the credentials file
		sed -e "s/friendica.example.net/${serverurl}/g" -e "s/_OVERRIDES_/${overrides}/g" -e "s/\(token=\${.*:-\"\)[0-9a-f]*\"/\1${credential}\"/g" "${i}" | tee "${k}" &>/dev/null
